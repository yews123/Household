...
<script>
  ...
  export default {
    ...
    beforeMount(){
      ...
      this.searchParams.keyword=this.$route.params.keyword||'';//后面得加||''不然重新挂载时this.$route.params.keyword为undefined与我写的接口对不上
    ...
    computed:{
      ...
      isTwo(){
        //如果order包含2，那么this.searchParams.order.indexOf('2')!=-1值就为true
    ...
    methods:{
      //向服务器发起请求获取search模块数据（根据参数不同返回不同的数据）
      getData(){
        this.$store.dispatch('getGoodsList',this.searchParams)
        this.$store.dispatch('getTrademarkList',this.searchParams)
      ...
      removeCategoryName(){
        ...
        //带给服务器参数是可有可无的：属性值为空的字符串还是会将相应的字段带给服务器
        //但是把属性值置为undefined,则当前字段不会带给服务器
        this.getData()
      ...
      attrInfo(attr,attrValue){
        ...
        if(this.searchParams.props.indexOf(props)==-1){
          //this.searchParams.props.indexOf(props)==-1说明数组里面没有这个属性
          this.searchParams.props.push(props)//数组
        }
        this.getData()
      },
      //删除售卖属性
      removeAttr(index){
      //再次整理参数
      this.searchParams.props.splice(index,1)//一次删除一个删除数组的索引值为index的那个值
      this.getData()
      },
      //排序的操作
      changeOrder(flag){
    ...
    watch:{
      //监听路由的信息是否发生变化，如果发生变化，再起发起请求
      $route(newValue,oldValue){
        //再次发起ajax请求前整理带给服务器的参数
        this.searchParams.category1Id=''
        this.searchParams.category2Id=''
        this.searchParams.category3Id=''
