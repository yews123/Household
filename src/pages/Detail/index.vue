      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          <div class="goodsDetail">
            <h3 class="InfoName">{{skuInfo[0].title}}</h3>
            <p class="news">{{skuInfo[0].msg}}</p>
            <div class="priceArea">
              <div class="priceArea1">
                <div class="title">价&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;格</div>
                <div class="price">
                  <i>¥</i>
                  <em>{{skuInfo[0].price}}</em>
                  <span>降价通知</span>
                </div>
                <div class="remark">
                  <i>累计评价</i>
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前咱们的路由跳转：从A路由跳转到B路由，这里加入购物车，进行路由跳转之前发请求，发请求
                把你购买的产品信息通过请求的形式通知服务器 -->
...
<script>
  ...
  export default {
    ...
    mounted(){
      ...
      this.$store.dispatch('getSkuImageList',this.$route.params.skuId)
    },
    data() {
      return {
        skuNum:1
    ...
    computed:{
      ...
      // //父组件给子组件传递的数据不能是undefined
      // skuImageList(){
      //   //当仓库还没获取到服务器的数据时this.skuInfo是一个空对象那么this.skuInfo.skuImageList为undefined.所以要||[]确保不为undefined
    ...
    methods:{
      ...
      async addShopcart(){
        ...
        /*
        当前这里是派发一个action，向服务器发请求，判断加入购物车是成功了还是失败了，进行相应的操作。
        this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuid,skuNum:this.skuNum})
        上面这行代码说白了：调用仓库中的addOrUpdateShopCart，这个方法加上了async，返回一定是Promise
        要么成功要么失败
        */
       //2.你需要知道这次请求是成功了还是失败了，如果成功进行路由跳转，如果失败需要给用户提示
       try {
        //成功
         await this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuId,skuNum:this.skuNum})
         //3.路由跳转
         //4.在路由跳转的时候还需要将产品的信息带给下一个路由组件
         //一些简单的数据skuNum,通过query或者params形式给路由组件传递过去
         //产品信息的数据（比较复杂：skuInfo(数组形式)）通过会话存储(不持久化，会话结束数据就会消失)
         //本地存储|会话存储，一般存储的是字符串
         sessionStorage.setItem("SKUINFO",JSON.stringify(this.skuInfo))
         this.$router.push({name:"addcartsucces",query:{skuNum:this.skuNum}})
       } 
       catch (error) {
        //失败
          alert(error.message)
